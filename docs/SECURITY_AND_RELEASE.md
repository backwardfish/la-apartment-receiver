...
Provider-contract concern, checked September 13: RentCast's [published listing schema](https://developers.rentcast.io/reference/property-listings-schema) does not document `listingUrl`, `imageUrl`/`photos`, or amenity descriptions used by this adapter. Listing-agent/office websites are not exact listing references. This is a documented schema gap and a likely product blocker, not a live-response observation. Restoring authentication does not clear it; the first actual response must establish usable evidence or acceptance stops.
...
2. Make one broad LA search such as `one bedroom under $4,500`. Check at least one real result's exact approved HTTPS source link, loaded photo, actual rent/bed/bath fields, retrieval timestamp, and availability wording. If the provider returns unusable links/photos or required fields, stop and record the product blocker. Do not substitute snapshots, synthesize source URLs, or claim live inventory passed.
3. Use bounded queries to exercise budget, bedrooms, parking, pets, and furnished requirements against the real evidence. Begin with `one bedroom under $3,200 with parking` and `pet friendly furnished one bedroom under $4,500`; adapt only to observed real records if needed, within the eight-reservation total. A zero result cannot by itself verify a feature mapping. Record any criterion that cannot be witnessed rather than treating it as passed.
